...
var Perl = {
  ...
  readyCallback: null,
  stdout_buf: "",
  stderr_buf: "", // for our default Perl.output implementation
...
window.addEventListener("load", function () {
  ...
  var scripts = [];
  var script_src;
  document.querySelectorAll("script[type='text/perl']").forEach(function (el) {
    if (el.src) {
      if (script_src || scripts.length)
        console.error(
          'Only a single Perl script may be loaded via "script src=", ignoring others',
        );
      else script_src = el.src;
    } else {
      if (script_src)
        console.error(
          'Only a single Perl script may be loaded via "script src=", ignoring others',
        );
      else scripts.push(el.innerHTML);
    }
  });
  if (script_src) {
    console.debug(
      "Perl: Found a script with src, fetching and running...",
      script_src,
    );
    ...
    if (!regex.exec(head)) {
      // load WebPerl unless the user loaded it
...
Perl.changeState = function (newState) {
  ...
  if (Perl.stateChanged) {
    console.info(
      "Perl.stateChanged is deprecated, please use Perl.addStateChangeListener instead",
    );
...
Perl.outputLine = function (chan, text) {
  // internal function
  if (arguments.length > 2)
    text = Array.prototype.slice.call(arguments, 1).join(" ");
  Perl.output(text, chan);
  Perl.output("\n", chan);
};
Perl.outputChar = function (chan, c) {
  // internal function
...
var getScriptURL = (function () {
  // with thanks to https://stackoverflow.com/a/2976714
...
Perl.Util.baseurl = function (urlstr) {
  ...
  if (url.protocol == "file:")
    ...
    return (
      url.origin + url.pathname.substring(0, url.pathname.lastIndexOf("/"))
    );
};
...
Perl.init = function (readyCallback) {
  ...
  Module = {
    noInitialRun: true,
    noExitRuntime: true,
    print: Perl.outputLine.bind(null, 1),
    printErr: Perl.outputLine.bind(null, 2),
    stdout: Perl.outputChar.bind(null, 1),
    stderr: Perl.outputChar.bind(null, 2),
    stdin: function () {
      return null;
    },
    arguments: ["--version"],
    ...
    onExit: function (status) {
      ...
      if (status == 0)
        ...
        alert(
          "Perl exited with exit status " + status + " in state " + Perl.state,
        );
      }
    ...
    locateFile: function (file) {
      ...
      if (wasmRe.exec(file)) return baseurl + "/" + file;
      return file;
    },
  };
...
Perl.initStepFinished = function () {
  if (Perl.state != "Initializing" || Perl.initStepsLeft < 1)
    throw (
      "Perl: internal error: can't call initStepFinished in state " +
      Perl.state +
      " (" +
      Perl.initStepsLeft +
      ")"
  ...
  if (--Perl.initStepsLeft) {
    console.debug(
      "Perl: One init step done, but " +
        Perl.initStepsLeft +
        " steps left, waiting...",
    );
    return;
  } else
    console.debug("Perl: All init steps done, doing final initialization...");
...
Perl.end = function () {
  ...
  try {
    ...
    if (status != 0)
      console.warn("emperl_end_perl returned with status", status);
...
Perl.unglue = function (id) {
  if (Perl.trace)
    console.debug("Perl: Unglue id", id, "from", Perl.GlueTable[id]);
